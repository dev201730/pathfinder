...
def a_star(grid, start, end):
    ...
    while len(open_set) > 0:
        ...
        for neighbor in current.get_neighbors():
            if neighbor in closed_set or neighbor.type == 'obstacle':
                continue
            # If both adjacent nodes are obstacles, dont let it be searched
            adj_node_1 = grid[current.y][neighbor.x]
            adj_node_2 = grid[neighbor.y][current.x]
            if adj_node_1.type == 'obstacle' and adj_node_2.type == 'obstacle':
                continue
